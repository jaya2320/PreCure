...
from .models import Post, Replie, Report,Doctor
...
def submit_forum(request):
    ...
    if request.method=="POST":   
        ...
        post = Post(user1=user, post_content=content,post_heading=heading,image=image)
        post.save()
        alert = "Your Query added Successfully !!"
        messages.info(request,alert)
        return redirect('forum')
    alert = "There is some problem while adding your query, please try again."
    messages.info(request,alert)
    return redirect('forum')


global post_selected_id
post_selected_id=1
...
def replyPage(request):
    ...
    image=str(post.image).split("/")
    image=image[-1]

    if request.method=="POST":
        
        post_reply=request.POST.get('post_reply')
        reply=Replie(user2=request.user,post=post,reply_content=post_reply)
        reply.save()

    return render(request,"forum_replies.html",{'post':post,'reply_count':replies.count,'replies':replies,"images":"media/post/"+image})



@login_required(login_url="welcome")
def forum_replies(request,id):
    global post_selected_id
    post_selected_id = id
    return redirect('replyPage')
